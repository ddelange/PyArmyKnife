...
def plt_setup(packages=(), preamble=(), tex=True, bold=None):
    ...
    if not preamble:
        ...
        preamble = [preamble]
    elif not isinstance(preamble, list):
        preamble = []

    packages.extend(["amsmath,wasysym"])
    preamble.extend(
        [
            r"\usepackage[detect-weight=true, detect-family=true, detect-inline-weight=math, detect-display-math]{siunitx}",
            r"\DeclareSIUnit\Msun{\ensuremath{\mathrm{M}_{\text{\footnotesize{\astrosun}}}}}",
        ]
    )
...
def save_figure(
    ...
    from pyarmyknife.fileio import open_with_default, current_path
    from matplotlib.pyplot import savefig, clf, close, gcf
    ...
    try:
        str(figname), str(printformat), int(dpi) < 5000, bool(open_fig)
        if figname.endswith(printformat):
            figname = figname[: -(len(printformat) + 1)]
    except Exception:
        logger.error("Wrong input for savefigure function.")
        return -1
    if path.exists(path.dirname(figname + "." + printformat)):
        finalname = figname + "." + printformat
    else:
        finalname = current_path + "img" + sep + figname + "." + printformat
    ...
    if clear:
        ...
        setup = False
    logger.info("Saved to '.../{}'.".format(sep.join((finalname).split(sep)[-3:])))
    if open:
        open_with_default(finalname)
    if setup:
        plt_setup()
    return finalname
...
def get_cmap(name):
    try:
        ...
        if name not in ["cividis", "cividis_r"]:
            raise
